...
//		System.out.print("\n");
		for(int r = 0; r < rows(); r++) {
			printRow(row(r)); System.out.print(" = "); labs.printRow(labs.row(r));
			System.out.println();
	...
	public void newColumn() { this.newColumn(0); }
	...
	public double[] popRow(int i) { return m_data.remove(i); }
	...
	public void pushRows(double[]... rows) {
		for(double[] row : rows) {
			if(row.length != cols())
				throw new IllegalArgumentException("Row size differs from the number of columns in this matrix.");
			m_data.add(row);
	...
	public boolean isColHomo(int col) {
		return colMAX(col) == colMIN(col);
	}
	...
	private static class SortComp implements Comparator<double[]> {
		int col;
		boolean ascend;
	
		SortComp(int col, boolean ascend) {
			this.col = col;
			this.ascend = ascend;
		}
		...
		public int compare(double[] a, double[] b) {
			int x = (ascend) ? -1 : 1;
			return (a[col] < b[col]) ? x*(1) : (a[col] > b[col]) ? (x*(-1)) : 0;
		}
	}

	public void sort(int column, boolean ascending) {
		m_data.sort(new SortComp(column, ascending));
	}
}
